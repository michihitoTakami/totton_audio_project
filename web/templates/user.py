...
<body>
    ...
    <div class="card">
        <div class="form-group">
            ...
            <select id="phaseType">
                ...
                <option value="hybrid" title="150Hz以下は最小位相＋上域は群遅延整形（約6.7ms整列＆音量一定）">Hybrid Phase (150Hz以下最小 / 150Hz以上整列)</option>
            </select>
            <div style="font-size:12px; color:#9fb3ff; margin-top:8px; line-height:1.4;">
                ハイブリッドは最小位相クロスオーバーに群遅延整形オールパスを重ね、150Hz以下は最小位相、150Hz以上は約6.7msで整列した位相となります。通過帯域は自動正規化され、位相タイプや倍率に依らず音量が一定です。
            </div>
        </div>
        <div id="phaseWarning" class="warning-banner">
            ⚠️ ハイブリッドは150Hz以下を最小位相、150Hz以上を群遅延整形（約6.7ms整列）で処理します。整列ディレイにより低遅延モードとは併用できませんが、音量は最小位相と同じ基準に自動調整されます。
        </div>
    ...
    <script>
        ...
        const HYBRID_PHASE_NOTE = 'ハイブリッドは最小位相クロスオーバーに群遅延整形を重ね、150Hz以下は最小位相、150Hz以上は約6.7msで整列しつつ音量を揃えます。';
        const HYBRID_PHASE_WARNING_TEXT = HYBRID_PHASE_NOTE + ' 整列ディレイのため低遅延モードとは併用できません。';
        ...
        function updatePhaseOptionAvailability() {
            ...
            hybridOption.title = isLowLatencyModeEnabled
                ? '低遅延モード中は選択できません'
                : '150Hz以下は最小位相＋上域は群遅延整形（約6.7ms整列＆音量一定）';
        }
