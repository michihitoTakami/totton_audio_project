...
<body>
    ...
    <div class="card">
        <div class="form-group">
            ...
            <select id="phaseType">
                ...
                <option value="hybrid" title="150Hz以下は最小位相 / 150Hz以上は線形位相（約6.7ms整列）">Hybrid Phase (150Hz以下最小 / 150Hz以上線形)</option>
            </select>
            <div style="font-size:12px; color:#9fb3ff; margin-top:8px; line-height:1.4;">
                ハイブリッドは150Hz以下を最小位相、150Hz以上を線形位相（約6.7ms整列）で処理し、定位を維持しながら低域のプレリンギングを抑えます。
            </div>
        </div>
        <div id="phaseWarning" class="warning-banner">
            ⚠️ ハイブリッドは150Hz以下を最小位相、150Hz以上を線形位相（約6.7ms整列）で処理します。約6.7msの整列ディレイが発生し、低遅延モードとは併用できません。
        </div>
    ...
    <script>
        ...
        const HYBRID_PHASE_NOTE = 'ハイブリッドは150Hz以下を最小位相、150Hz以上を線形位相（約6.7ms整列）で処理します。';
        const HYBRID_PHASE_WARNING_TEXT = HYBRID_PHASE_NOTE + ' 約6.7msの整列ディレイが発生し、低遅延モードとは併用できません。';
        ...
        function updatePhaseOptionAvailability() {
            ...
            hybridOption.title = isLowLatencyModeEnabled ? '低遅延モード中は選択できません' : '150Hz以下は最小位相 / 150Hz以上は線形位相（約6.7ms整列）';
        }
