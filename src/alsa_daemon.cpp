...
static nlohmann::json collect_runtime_stats_json() {
    ...
    if (g_upsampler) {
        phaseTypeStr = (g_upsampler->getPhaseType() == PhaseType::Minimum) ? "minimum" : "hybrid";
    } else {
        phaseTypeStr = (g_config.phaseType == PhaseType::Minimum) ? "minimum" : "hybrid";
    }
...
static std::string resolve_filter_path_for(ConvolutionEngine::RateFamily family, PhaseType phase) {
    if (phase == PhaseType::Hybrid) {
        return (family == ConvolutionEngine::RateFamily::RATE_44K) ? g_config.filterPath44kHybrid
                                                                   : g_config.filterPath48kHybrid;
    }
...
static std::string handle_phase_type_get(const daemon_ipc::ZmqRequest& request) {
    ...
    std::string ptStr = (pt == PhaseType::Minimum) ? "minimum" : "hybrid";
    nlohmann::json data;
...
static std::string handle_phase_type_set(const daemon_ipc::ZmqRequest& request) {
    ...
    if (phaseStr.empty()) {
        return build_error_response(request, "IPC_INVALID_PARAMS",
                                    "Invalid phase type (use 'minimum' or 'hybrid')");
    }
    ...
    if (phaseStr != "minimum" && phaseStr != "hybrid") {
        return build_error_response(request, "IPC_INVALID_PARAMS",
                                    "Invalid phase type (use 'minimum' or 'hybrid')");
    }

    PhaseType newPhase = (phaseStr == "minimum") ? PhaseType::Minimum : PhaseType::Hybrid;
    ...
    if (newPhase == PhaseType::Hybrid && g_config.partitionedConvolution.enabled) {
    ...
    do {
        ...
        if (g_config.multiRateEnabled) {
            ...
            for (const auto& path : {g_config.filterPath44kMin, g_config.filterPath48kMin,
                                     g_config.filterPath44kHybrid, g_config.filterPath48kHybrid}) {
            ...
            initSuccess = g_upsampler->initializeQuadPhase(
                g_config.filterPath44kMin, g_config.filterPath48kMin, g_config.filterPath44kHybrid,
                g_config.filterPath48kHybrid, g_config.upsampleRatio, g_config.blockSize,
        ...
        // Log latency note for hybrid phase
        if (g_config.phaseType == PhaseType::Hybrid) {
