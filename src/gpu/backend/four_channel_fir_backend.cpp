...
BackendFourChannelFIR::BackendFourChannelFIR(BackendFactory factory, std::string name)
    : backendFactory_(std::move(factory)), name_(std::move(name)) {
    activeFilterFFT_.fill(nullptr);
    configLoaded_.fill(false);
}

BackendFourChannelFIR::~BackendFourChannelFIR() {
    cleanup();
}

const std::string& BackendFourChannelFIR::tag() const {
    return name_;
}

int BackendFourChannelFIR::getFamilyIndex(ConvolutionEngine::RateFamily family) const {
    return rateFamilyToIndex(family);
}

int BackendFourChannelFIR::getConfigIndex(ConvolutionEngine::HeadSize size,
...
bool BackendFourChannelFIR::loadHrtfConfig(const std::string& binPath, const std::string& jsonPath,
    ...
    try {
        ...
        LOG_ERROR("[{}] Failed to parse metadata {}: {}", tag(), jsonPath, e.what());
        return false;
    }
    ...
    if (channels != kChannelCount || taps <= 0) {
        LOG_ERROR("[{}] Invalid metadata in {} (channels={}, taps={})", tag(), jsonPath, channels,
                  taps);
        return false;
    }
    ...
    if (taps != enforcedTaps) {
        LOG_ERROR("[{}] Tap mismatch: {} expected {} got {}", tag(), binPath, enforcedTaps, taps);
        return false;
    }
    ...
    if (fileSize != expectedBytes) {
        LOG_ERROR("[{}] File size mismatch: {} expected {} got {}", tag(), binPath, expectedBytes,
                  fileSize);
        return false;
    }
    ...
    if (!binFile) {
        LOG_ERROR("[{}] Failed to read {}", tag(), binPath);
        return false;
    }
...
bool BackendFourChannelFIR::setupBackendResources() {
    ...
    auto check = [&](AudioEngine::ErrorCode ec, const char* context) -> bool {
        if (ec != AudioEngine::ErrorCode::OK) {
            LOG_ERROR("[{}] {} failed: {}", tag(), context, static_cast<int>(ec));
            return false;
        }
        return true;
    };
    ...
    if (activeIdx < 0) {
        LOG_ERROR("[{}] No valid filter configuration loaded", tag());
        return false;
    }
    for (int c = 0; c < kChannelCount; ++c) {
...
bool BackendFourChannelFIR::initialize(const std::string& hrtfDir, int blockSize,
    ...
    if (!backendFactory_) {
        LOG_ERROR("[{}] Backend factory not provided", tag());
        return false;
    }

    backend_ = backendFactory_();
    if (!backend_) {
        LOG_ERROR("[{}] Failed to create backend", tag());
        return false;
    }
    ...
    if (!anyLoaded) {
        LOG_ERROR("[{}] No HRTF files found under {}", tag(), hrtfDir);
        cleanup();
        return false;
    }
...
bool BackendFourChannelFIR::initializeStreaming() {
...
void BackendFourChannelFIR::resetStreaming() {
...
bool BackendFourChannelFIR::switchHeadSize(ConvolutionEngine::HeadSize targetSize) {
...
bool BackendFourChannelFIR::switchRateFamily(ConvolutionEngine::RateFamily targetFamily) {
...
bool BackendFourChannelFIR::processStreamBlock(
    ...
    if (!initialized_ || !streamInitialized_ || !backend_) {
        outputL.clear();
        outputR.clear();
        return false;
    }
    ...
    if (requiredL > streamInputBufferL.size() || requiredR > streamInputBufferR.size()) {
        ...
        LOG_EVERY_N(ERROR, 100,
                    "[{}] Input buffer too small (requiredL={}, requiredR={}, sizeL={}, sizeR={}, "
                    "inputFrames={}). Dropping accumulated stream state (keep={} frames).",
                    tag(), requiredL, requiredR, sizeL, sizeR, inputFrames, kept);
    ...
    if (outputL.capacity() < static_cast<size_t>(validOutputPerBlock_) ||
        ...
        LOG_EVERY_N(ERROR, 100, "[{}] Output buffer capacity too small", tag());
        outputL.clear();
        outputR.clear();
        return false;
    }
    ...
    auto mul = [&](DeviceBuffer& out, const DeviceBuffer& a, const DeviceBuffer* filter,
        ...
        if (!filter || !filter->handle.ptr) {
            LOG_ERROR("[{}] Active filter missing for {}", tag(), label);
            return false;
        }
...
size_t BackendFourChannelFIR::getStreamValidInputPerBlock() const {
    return streamValidInputPerBlock_;
}

size_t BackendFourChannelFIR::getValidOutputPerBlock() const {
    return static_cast<size_t>(validOutputPerBlock_);
}

int BackendFourChannelFIR::getFilterTaps() const {
    return filterTaps_;
}

ConvolutionEngine::HeadSize BackendFourChannelFIR::getCurrentHeadSize() const {
    return currentHeadSize_;
}

ConvolutionEngine::RateFamily BackendFourChannelFIR::getCurrentRateFamily() const {
    return currentRateFamily_;
}

bool BackendFourChannelFIR::isInitialized() const {
    return initialized_;
}

void BackendFourChannelFIR::setEnabled(bool enabled) {
    enabled_ = enabled;
}

bool BackendFourChannelFIR::isEnabled() const {
    return enabled_;
}

void BackendFourChannelFIR::cleanup() {
    if (backend_) {
...
CudaFourChannelFIR::CudaFourChannelFIR()
    : BackendFourChannelFIR(createCudaBackend, "CudaFourChannelFIR") {}
#endif
