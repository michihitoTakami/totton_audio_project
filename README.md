...
**ヘッドルーム設定**: `headroomTarget`（デフォルト`0.92`）は出力リミッターのターゲットピークを線形値で指定します。`headroomMode`（デフォルト`family_max`）が同一レートファミリ内の入力帯域ピーク最大値を使って`safeGain`を固定するため、線形/最小位相や2x/4x/8x/16x切替でも音量差を最小化できます。旧挙動に戻す場合は`per_filter`を指定してください。0.5–0.999の範囲で設定可能です。
