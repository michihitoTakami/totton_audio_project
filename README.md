...
**ヘッドルーム設定**: `headroomTarget`（デフォルト`0.92`）は出力リミッターのターゲットピークを線形値で指定します。フィルタメタデータの入力帯域ピーク（`input_band_peak_normalized`があればそれ、無ければ`normalized_dc_gain / upsample_ratio`）を基準にヘッドルームゲインを算出するため、線形/最小位相や倍率切替でも音量ジャンプを抑えつつ、ピークを0dBFS未満に抑えてクリッピングを防ぎます。0.5–0.999の範囲で設定可能です。
