...
// Backend-agnostic 4ch FIR (LL/LR/RL/RR) implemented on top of IGpuBackend.
class BackendFourChannelFIR : public ConvolutionEngine::IFourChannelFIR {
   public:
    using BackendFactory = std::function<std::unique_ptr<IGpuBackend>()>;

    BackendFourChannelFIR(BackendFactory factory, std::string name);
    ~BackendFourChannelFIR() override;

    BackendFourChannelFIR(const BackendFourChannelFIR&) = delete;
    BackendFourChannelFIR& operator=(const BackendFourChannelFIR&) = delete;
...
class CudaFourChannelFIR : public BackendFourChannelFIR {
   public:
    CudaFourChannelFIR();
};
#endif
