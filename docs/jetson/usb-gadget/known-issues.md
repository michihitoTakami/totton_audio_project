...
```
PC  ──USB──>  USB-DDC  ──S/PDIF──>  Totton Audio  ──USB──>  DAC
                         (同軸/光)
```
...
```
                    ┌─────────────────────────┐
PC  ──USB──>  DAC  ─┤ I2S (BCLK, LRCK, DATA) ├──>  Totton Audio (GPIO)
                    └─────────────────────────┘
```
...
> ⚠️ RTP経路は現在 GStreamer RTP入力に一本化されています。
...
```
PC  ──USB──>  USB Audio Interface  ──USB──>  Totton Audio (Host)  ──>  DAC
              (UAC2対応)
```
