...
Totton Audio Projectは**Control Plane / Data Plane分離アーキテクチャ**を採用しています。
...
**エンドポイント**: `ipc:///tmp/totton_audio.sock`
...
電源ON
   ...
   │       │
   │       ├─> totton-audio-gadget.service     [USB Gadget初期化]
   │       │       │
   │       │       ├─> ConfigFS設定
   │       │       └─> UDC有効化
   │       │
   ...
   │       │       └─> 入力待機
   │       │
   │       └─> totton-audio-web.service        [Web UI]
   │               │
   │               └─> uvicorn :80 起動
   │
   └─> Ready (約30秒)
