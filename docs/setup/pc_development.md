...
Jetson では `rtp_input` サービス（GStreamer）で RTP を受信し、ALSA Loopback playback に書き込みます。Totton Audio Project Web API から起動できます。
...
[Service]
Type=simple
WorkingDirectory=/path/to/totton_audio
