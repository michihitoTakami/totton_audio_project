...
## 今後のTODO（Web/API）
- 認証とバインド範囲: localhost 前提を維持しつつ、簡易トークン/BASIC 認証をオプション化。LAN 公開時のリスク低減。
- 状態の一元化: `/status` でデーモン実プロセスの有効設定・クリップ統計を返す仕組み（例: Unix ドメインソケット経由の照会）を追加し、config.json との乖離を防ぐ。
- 再起動/起動経路: `/restart` で未起動時の起動や失敗時のリトライ結果をレスポンスに含める。起動方法(systemd/直接起動)を選択できるようにする。
- エラーレポート: フィルタ欠落、ALSA オープン失敗などを API レスポンスに載せ、UI が明示的に提示できるようにする。
- UI 改善: 「保存のみ」と「保存＋リロード」を分離、処理中インジケータや成功/失敗トーストを整理し、リロード結果を可視化。
- EQ 機能: `/eq-profile` のプレースホルダに対し、FIR 生成・メタデータ更新までの具体的な実装方針を固める。
