...
def generate_sweep(duration: float = 5.0, fs: int = 44100, f_start: float = 20, f_end: float = 20000) -> np.ndarray:
...
def analyze_frequency_response(signal_data: np.ndarray, fs: int, nperseg: int = 8192) -> tuple:
...
def process_with_convolution_engine(input_signal: np.ndarray, eq_enabled: bool,
                                     eq_profile: str, filter_path: str,
                                     build_dir: Path) -> np.ndarray:
    """
...
def compute_eq_response_python(freqs: np.ndarray, eq_file: str, fs: float) -> np.ndarray:
    ...
    with open(eq_file, 'r') as f:
        for line in f:
            ...
            preamp_match = re.match(r'Preamp:\s*([-+]?\d*\.?\d+)\s*dB?', line, re.IGNORECASE)
            if preamp_match:
...
def run_gpu_test(input_wav: str, output_wav: str, eq_enabled: bool, eq_profile: str, build_dir: Path):
...
def main():
    ...
    ax1 = axes[0, 0]
    ax1.semilogx(freqs_sweep[freqs_sweep > 10], psd_sweep[freqs_sweep > 10],
                  label='Sweep', alpha=0.7)
    ax1.semilogx(freqs_noise[freqs_noise > 10], psd_noise[freqs_noise > 10],
    ...
    ax2 = axes[0, 1]
    ax2.semilogx(test_freqs, eq_response, 'b-', linewidth=2, label='Expected EQ Response')
            ...
            if line.strip().startswith('Filter') and 'ON' in line:
                ...
                if match:
                    ...
                    if abs(gain) > 3:  # Only annotate significant changes
                        ax4.annotate(f'{gain:+.0f}dB', (fc, gain), fontsize=8, alpha=0.7)

    ax4.semilogx(test_freqs, eq_response, 'b-', linewidth=2)
    ax4.fill_between(test_freqs, 0, eq_response, alpha=0.3,
                      where=eq_response > 0, color='green', label='Boost')
    ax4.fill_between(test_freqs, 0, eq_response, alpha=0.3,
                      where=eq_response < 0, color='red', label='Cut')
    ...
    print(f"** Play these through the system to verify EQ effect **")
    print("=" * 60)
