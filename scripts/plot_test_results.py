...
def main():
    ...
    ax3.fill_between(freq_diff, 0, mag_diff,
                     where=mag_diff > 0, alpha=0.3, color='green', label='Boost')
    ax3.fill_between(freq_diff, 0, mag_diff,
                     where=mag_diff < 0, alpha=0.3, color='red', label='Cut')
    ...
    print(f"  Max boost: {mag_diff[max_boost_idx]:.1f} dB at {freq_diff[max_boost_idx]:.0f} Hz")
    print(f"  Max cut:   {mag_diff[max_cut_idx]:.1f} dB at {freq_diff[max_cut_idx]:.0f} Hz")
