...
def calculate_biquad_coeffs(filter_type: str, fc: float, gain_db: float, q: float, fs: float) -> tuple:
    """
...
def biquad_frequency_response(coeffs: tuple, freqs: np.ndarray, fs: float) -> np.ndarray:
...
def compute_eq_response(preamp_db: float, bands: list[dict], freqs: np.ndarray, fs: float) -> np.ndarray:
...
def plot_eq_response(eq_file: str, output_file: str = None, sample_rate: float = 44100):
    ...
    for i, band in enumerate(bands):
        print(f"    {i+1}: {band['type']} Fc={band['fc']:.1f}Hz Gain={band['gain']:.1f}dB Q={band['q']:.2f}")
    ...
    # Add preamp line
    ax.axhline(y=preamp_db, color='r', linestyle='--', alpha=0.5, label=f'Preamp ({preamp_db} dB)')
...
def compare_eq_profiles(eq_files: list[str], output_file: str = None, sample_rate: float = 44100):
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        if len(eq_files) > 1:
            compare_eq_profiles(
                [str(f) for f in eq_files],
                str(output_dir / 'eq_comparison.png')
            )
