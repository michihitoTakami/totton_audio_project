...
VkFFTResult sample_1002_benchmark_VkFFT_half_2_4096(VkGPU* vkGPU, uint64_t file_output,
    ...
    if (file_output)
        fprintf(output,
                "1002 - VkFFT FFT + iFFT C2C benchmark 1D batched in half precision: all supported "
                "systems from 2 to 4096\n");
    printf(
        "1002 - VkFFT FFT + iFFT C2C benchmark 1D batched in half precision: all supported systems "
        "from 2 to 4096\n");
    ...
    for (uint64_t n = 1; n < 4097; n++) {
        ...
        for (uint64_t r = 0; r < num_runs; r++) {
            ...
            configuration.numberBatches = (uint64_t)pow(
                2,
                (uint64_t)log2((uint64_t)64 * 32 * (uint64_t)pow(2, 16) / configuration.size[0]));
            ...
            uint64_t bufferSize =
                (uint64_t)sizeof(half) * 2 * configuration.size[0] * configuration.numberBatches;
#if (VKFFT_BACKEND == 0)
            VkBuffer buffer = {};
            ...
            res = zeMemAllocDevice(vkGPU->context, &device_desc, bufferSize, sizeof(half),
                                   vkGPU->device, &buffer);
            ...
            */
            ...
            // Submit FFT+iFFT.
            uint64_t num_iter = (((uint64_t)3 * 4096 * 1024.0 * 1024.0) / bufferSize > 1000)
                                    ? 1000
                                    : (uint64_t)((uint64_t)3 * 4096 * 1024.0 * 1024.0) / bufferSize;
#if (VKFFT_BACKEND == 0)
                ...
                if (r == num_runs - 1) {
                    ...
                    if (file_output)
                        fprintf(
                            ...
                            (uint64_t)(((double)bufferSize / 512) / avg_time),
