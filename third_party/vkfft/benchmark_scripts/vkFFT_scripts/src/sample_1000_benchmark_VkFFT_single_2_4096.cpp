...
VkFFTResult sample_1000_benchmark_VkFFT_single_2_4096(VkGPU* vkGPU, uint64_t file_output,
                                                      FILE* output,
    ...
    if (file_output)
        fprintf(output,
                "1000 - VkFFT FFT + iFFT C2C benchmark 1D batched in single precision: all "
                "supported systems from 2 to 4096\n");
    printf(
        "1000 - VkFFT FFT + iFFT C2C benchmark 1D batched in single precision: all supported "
    ...
    for (uint64_t n = 1; n < 4097; n++) {
        ...
        for (uint64_t r = 0; r < num_runs; r++) {
            ...
            uint64_t bufferSize =
                (uint64_t)sizeof(float) * 2 * configuration.size[0] * configuration.numberBatches;
#if (VKFFT_BACKEND == 0)
            VkBuffer buffer = {};
                ...
                if (r == num_runs - 1) {
                    ...
                    if (file_output)
                        fprintf(
                            ...
                            (uint64_t)(((double)bufferSize / 1024) / avg_time),
