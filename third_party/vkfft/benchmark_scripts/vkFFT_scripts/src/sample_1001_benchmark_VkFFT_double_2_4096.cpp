...
VkFFTResult sample_1001_benchmark_VkFFT_double_2_4096(VkGPU* vkGPU, uint64_t file_output,
                                                      FILE* output,
    ...
    if (file_output)
        fprintf(output,
                "1001 - VkFFT FFT + iFFT C2C benchmark 1D batched in double precision: all "
                "supported systems from 2 to 4096\n");
    printf(
        "1001 - VkFFT FFT + iFFT C2C benchmark 1D batched in double precision: all supported "
    ...
    for (int n = 1; n < 4097; n++) {
        ...
        for (uint64_t r = 0; r < num_runs; r++) {
            ...
            configuration.numberBatches = (uint64_t)pow(
                2,
                (uint64_t)log2((uint64_t)64 * 32 * (uint64_t)pow(2, 15) / configuration.size[0]));
            ...
            uint64_t bufferSize =
                (uint64_t)sizeof(double) * 2 * configuration.size[0] * configuration.numberBatches;
#if (VKFFT_BACKEND == 0)
            VkBuffer buffer = {};
                ...
                if (r == num_runs - 1) {
                    ...
                    if (file_output)
                        fprintf(
                            ...
                            (uint64_t)(((double)bufferSize * sizeof(float) / sizeof(double) /
                                        1024) /
                                       avg_time),
                    ...
                    printf("VkFFT System: %" PRIu64 " %" PRIu64 " Buffer: %" PRIu64
                           ...
                           configuration.size[0], configuration.numberBatches,
                           bufferSize / 1024 / 1024, avg_time, std_error, num_iter,
                           (uint64_t)(((double)bufferSize * sizeof(float) / sizeof(double) / 1024) /
                                      avg_time),
                           bufferSize / 1024.0 / 1024.0 / 1.024 * 4 / avg_time,
                           bufferSize / 1024.0 / 1024.0 / 1.024 * num_tot_transfers / avg_time);
                    benchmark_result +=
                        ((double)bufferSize * sizeof(float) / sizeof(double) / 1024) / avg_time;
                }
            }
