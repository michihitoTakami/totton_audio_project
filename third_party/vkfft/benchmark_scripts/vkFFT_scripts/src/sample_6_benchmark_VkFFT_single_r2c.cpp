...
VkFFTResult sample_6_benchmark_VkFFT_single_r2c(VkGPU* vkGPU, uint64_t file_output, FILE* output,
    ...
    uint64_t benchmark_dimensions[num_benchmark_samples][4] = {
        {1024, 1024, 1, 2}, {64, 64, 1, 2},     {256, 256, 1, 2},    {1024, 256, 1, 2},
        {512, 512, 1, 2},   {1024, 1024, 1, 2}, {4096, 256, 1, 2},   {2048, 1024, 1, 2},
        {4096, 2048, 1, 2}, {4096, 4096, 1, 2}, {720, 480, 1, 2},    {1280, 720, 1, 2},
        {1920, 1080, 1, 2}, {2560, 1440, 1, 2}, {3840, 2160, 1, 2},  {32, 32, 32, 3},
        {64, 64, 64, 3},    {256, 256, 32, 3},  {1024, 256, 32, 3},  {256, 256, 256, 3},
        {2048, 1024, 8, 3}, {512, 512, 128, 3}, {2048, 256, 256, 3}, {4096, 512, 8, 3}};
    ...
    for (uint64_t n = 0; n < num_benchmark_samples; n++) {
        ...
        for (uint64_t r = 0; r < num_runs; r++) {
            ...
            configuration.performR2C =
                true;  // Perform R2C/C2R transform. Can be combined with all other options. Reduces
                       // memory requirements by a factor of 2. Requires special input data
                       // alignment: for x*y*z system pad x*y plane to (x+2)*y with last 2*y
                       // elements reserved, total array dimensions are (x*y+2y)*z. Memory layout
                       // after R2C and before C2R can be found on github.
            ...
            for (uint64_t k = 0; k < configuration.size[2]; k++) {
                for (uint64_t j = 0; j < configuration.size[1]; j++) {
                    for (uint64_t i = 0; i < configuration.size[0]; i++) {
                        buffer_input[i + j * configuration.size[0] + k * (configuration.size[0] + 2)
            * configuration.size[1]] = i;//[-1,1]
                        }
                    }
                }
            */
            ...
            /*
                for (uint64_t k = 0; k < configuration.size[2]; k++) {
                    for (uint64_t j = 0; j < configuration.size[1]; j++) {
                        for (uint64_t i = 0; i < configuration.size[0]; i++) {
                            printf("%.6f ", buffer_output[i + j * configuration.size[0] + k *
               (configuration.size[0] + 2) * configuration.size[1] + v * (configuration.size[0] + 2)
               * configuration.size[1] * configuration.size[2]]);
                        }
                        std::cout << "\n";
                    }
