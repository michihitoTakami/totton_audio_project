...
VkFFTResult sample_2_benchmark_VkFFT_half(VkGPU* vkGPU, uint64_t file_output, FILE* output,
    ...
    for (uint64_t n = 0; n < 25; n++) {
        ...
        for (uint64_t r = 0; r < num_runs; r++) {
            ...
            uint64_t bufferSize =
                (uint64_t)2 * sizeof(half) * configuration.size[0] * configuration.numberBatches;
#if (VKFFT_BACKEND == 0)
            VkBuffer buffer = {};
                ...
                if (r == num_runs - 1) {
                    ...
                    if (file_output)
                        fprintf(
                            ...
                            std_error, num_iter,
                            (uint64_t)(((double)bufferSize * sizeof(float) / sizeof(half) / 1024) /
                                       avg_time),
                    ...
                    printf("VkFFT System: %" PRIu64 " %" PRIu64 "x%" PRIu64 " Buffer: %" PRIu64
                           ...
                           bufferSize / 1024.0 / 1024.0 / 1.024 * num_tot_transfers / avg_time);
                    benchmark_result +=
                        ((double)bufferSize * sizeof(float) / sizeof(half) / 1024) / avg_time;
                }
            }
