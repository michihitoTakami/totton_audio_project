...
VkFFTResult user_benchmark_VkFFT(VkGPU* vkGPU, uint64_t file_output, FILE* output,
    ...
    if (userParams->R2C) {
        bufferSize = (uint64_t)(storageComplexSize / 2) * (userParams->X + 2) * userParams->Y *
                     userParams->Z * userParams->B;
    } else {
        if (userParams->DCT) {
            bufferSize = (uint64_t)(storageComplexSize / 2) * userParams->X * userParams->Y *
                         userParams->Z * userParams->B;
        } else {
            bufferSize = (uint64_t)storageComplexSize * userParams->X * userParams->Y *
                         userParams->Z * userParams->B;
        }
    }
    ...
    for (uint64_t n = 0; n < 2; n++) {
        ...
        for (uint64_t r = 0; r < num_runs; r++) {
...
#if (VKFFT_BACKEND != 5)
            if (userParams->saveApplicationToString && (n == 0) && (r == 0))
                configuration.saveApplicationToString = 1;
            if (userParams->loadApplicationFromString ||
                (userParams->saveApplicationToString && ((n != 0) || (r != 0))))
            ...
            if (configuration.loadApplicationFromString) {
                ...
                sprintf(fname,
                        "VkFFT_binary_X%" PRIu64 "_Y%" PRIu64 "_Z%" PRIu64 "_P%" PRIu64 "_B%" PRIu64
                        "_N%" PRIu64 "_R2C%" PRIu64 "_DCT%" PRIu64 "_ver%d",
                        userParams->X, userParams->Y, userParams->Z, userParams->P, userParams->B,
                        userParams->N, userParams->R2C, userParams->DCT, VkFFT_version);
            ...
            if (n > 0) {
                if (r == num_runs - 1) {
                    ...
                    if (file_output)
                        fprintf(
                            output,
                            "VkFFT System: %" PRIu64 "x%" PRIu64 "x%" PRIu64 " Batch: %" PRIu64
                            " Buffer: %" PRIu64
                            " MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %" PRIu64
                            " benchmark: %" PRIu64 " scaled bandwidth: %0.1f bandwidth: %0.1f\n",
                            configuration.size[0], configuration.size[1], configuration.size[2],
                            userParams->B, bufferSize / 1024 / 1024, avg_time, std_error,
                            userParams->N, (uint64_t)(((double)bufferSize / 1024) / avg_time),
                            bufferSize / 1024.0 / 1024.0 / 1.024 * 4 * configuration.FFTdim /
                                avg_time,
                    ...
                    printf(
                        ...
                        " Buffer: %" PRIu64
                        " MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %" PRIu64
                        " benchmark: %" PRIu64 " scaled bandwidth: %0.1f real bandwidth: %0.1f\n",
                        configuration.size[0], configuration.size[1], configuration.size[2],
                        userParams->B, bufferSize / 1024 / 1024, avg_time, std_error, userParams->N,
                        (uint64_t)(((double)bufferSize / 1024) / avg_time),
                        bufferSize / 1024.0 / 1024.0 / 1.024 * 4 * configuration.FFTdim / avg_time,
                        bufferSize / 1024.0 / 1024.0 / 1.024 * num_tot_transfers / avg_time);
