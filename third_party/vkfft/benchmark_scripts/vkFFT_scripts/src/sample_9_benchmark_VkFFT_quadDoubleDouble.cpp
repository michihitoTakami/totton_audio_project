...
VkFFTResult sample_9_benchmark_VkFFT_quadDoubleDouble(VkGPU* vkGPU, uint64_t file_output,
                                                      FILE* output,
    ...
    if (file_output)
        fprintf(output,
                "9 - VkFFT FFT + iFFT C2C benchmark 1D batched in double-double emulation of quad "
                "precision LUT\n");
    printf(
        "9 - VkFFT FFT + iFFT C2C benchmark 1D batched in double-double emulation of quad "
        "precision LUT\n");
    ...
    for (uint64_t n = 0; n < 23; n++) {
        ...
        for (uint64_t r = 0; r < num_runs; r++) {
            ...
            if (n == 0)
                configuration.size[0] = 2048;
            configuration.numberBatches =
                (uint64_t)(64 * 16 * (uint64_t)pow(2, 14)) / configuration.size[0];
                ...
                if (r == num_runs - 1) {
                    ...
                    if (file_output)
                        fprintf(
                            ...
                            configuration.numberBatches, bufferSize / 1024 / 1024, avg_time,
                            std_error, num_iter,
