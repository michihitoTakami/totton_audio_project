...
VkResult CreateDebugUtilsMessengerEXT(VkGPU* vkGPU,
    ...
    PFN_vkCreateDebugUtilsMessengerEXT func =
        (PFN_vkCreateDebugUtilsMessengerEXT)vkGetInstanceProcAddr(vkGPU->instance,
...
void DestroyDebugUtilsMessengerEXT(VkGPU* vkGPU, const VkAllocationCallbacks* pAllocator) {
    ...
    PFN_vkDestroyDebugUtilsMessengerEXT func =
        (PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(
            vkGPU->instance, "vkDestroyDebugUtilsMessengerEXT");
    if (func != NULL) {
...
VkResult setupDebugMessenger(VkGPU* vkGPU) {
    ...
    if (vkGPU->enableValidationLayers == 0)
        return VK_SUCCESS;
    ...
    if (CreateDebugUtilsMessengerEXT(vkGPU, &createInfo, NULL, &vkGPU->debugMessenger) !=
        VK_SUCCESS) {
...
VkResult checkValidationLayerSupport() {
    ...
    VkLayerProperties* availableLayers =
        (VkLayerProperties*)malloc(sizeof(VkLayerProperties) * layerCount);
    if (!availableLayers)
...
VkResult getComputeQueueFamilyIndex(VkGPU* vkGPU) {
    ...
    if (!queueFamilies)
        return VK_INCOMPLETE;
    if (queueFamilies) {
        vkGetPhysicalDeviceQueueFamilyProperties(vkGPU->physicalDevice, &queueFamilyCount,
                                                 queueFamilies);
        uint64_t i = 0;
...
VkFFTResult findMemoryType(VkGPU* vkGPU, uint64_t memoryTypeBits, uint64_t memorySize,
    ...
    for (uint64_t i = 0; i < memoryProperties.memoryTypeCount; ++i) {
        if ((memoryTypeBits & ((uint64_t)1 << i)) &&
            ...
            (memoryProperties.memoryHeaps[memoryProperties.memoryTypes[i].heapIndex].size >=
             memorySize)) {
...
VkFFTResult allocateBuffer(VkGPU* vkGPU, VkBuffer* buffer, VkDeviceMemory* deviceMemory,
    ...
    resFFT = findMemoryType(vkGPU, memoryRequirements.memoryTypeBits, memoryRequirements.size,
                            propertyFlags, &memoryAllocateInfo.memoryTypeIndex);
...
#endif
VkFFTResult transferDataToCPU(VkGPU* vkGPU, void* cpu_arr, void* output_buffer,
                              uint64_t transferSize) {
    // a function that transfers data from the GPU to the CPU using staging buffer, because the GPU
    ...
    if (!vkGPU->stagingBuffer) {
        ...
        resFFT = allocateBuffer(
            vkGPU, stagingBuffer, stagingBufferMemory,
    ...
    res = clEnqueueReadBuffer(commandQueue, buffer[0], CL_TRUE, 0, transferSize, cpu_arr, 0, NULL,
                              NULL);
    if (res != CL_SUCCESS) {
        return VKFFT_ERROR_FAILED_TO_COPY;
    }
    ...
    void* buffer = ((void**)output_buffer)[0];
    ze_command_queue_desc_t commandQueueCopyDesc = {ZE_STRUCTURE_TYPE_COMMAND_QUEUE_DESC,
                                                    0,
    ...
    res = zeCommandListCreateImmediate(vkGPU->context, vkGPU->device, &commandQueueCopyDesc,
                                       &copyCommandList);
    ...
    MTL::Buffer* stagingBuffer =
        vkGPU->device->newBuffer(transferSize, MTL::ResourceStorageModeShared);
    ...
    blitCommandEncoder->copyFromBuffer((MTL::Buffer*)buffer, 0, (MTL::Buffer*)stagingBuffer, 0,
                                       transferSize);
...
VkFFTResult transferDataFromCPU(VkGPU* vkGPU, void* cpu_arr, void* input_buffer,
                                uint64_t transferSize) {
    VkFFTResult resFFT = VKFFT_SUCCESS;
#if (VKFFT_BACKEND == 0)
    VkResult res = VK_SUCCESS;
    ...
    if (res != CL_SUCCESS)
        return VKFFT_ERROR_FAILED_TO_CREATE_COMMAND_QUEUE;
    res = clEnqueueWriteBuffer(commandQueue, buffer[0], CL_TRUE, 0, transferSize, cpu_arr, 0, NULL,
                               NULL);
    if (res != CL_SUCCESS) {
        return VKFFT_ERROR_FAILED_TO_COPY;
    }
...
#elif (VKFFT_BACKEND == 5)
    MTL::Buffer* stagingBuffer =
        vkGPU->device->newBuffer(cpu_arr, transferSize, MTL::ResourceStorageModeShared);
    MTL::CommandBuffer* copyCommandBuffer = vkGPU->queue->commandBuffer();
    if (copyCommandBuffer == 0)
        return VKFFT_ERROR_FAILED_TO_CREATE_COMMAND_LIST;
    MTL::BlitCommandEncoder* blitCommandEncoder = copyCommandBuffer->blitCommandEncoder();
    ...
    blitCommandEncoder->copyFromBuffer((MTL::Buffer*)stagingBuffer, 0, (MTL::Buffer*)buffer, 0,
                                       transferSize);
    blitCommandEncoder->endEncoding();
    copyCommandBuffer->commit();
    copyCommandBuffer->waitUntilCompleted();
...
VkFFTResult devices_list() {
    ...
    if (devices) {
        ...
        for (uint64_t i = 0; i < deviceCount; i++) {
            ...
            printf("Device id: %" PRIu64 " name: %s API:%d.%d.%d\n", i,
                   device_properties.deviceName, (device_properties.apiVersion >> 22),
    ...
    for (uint64_t j = 0; j < numPlatforms; j++) {
        ...
        for (uint64_t i = 0; i < numDevices; i++) {
            ...
            if (res != CL_SUCCESS)
                return VKFFT_ERROR_FAILED_TO_ENUMERATE_DEVICES;
            res = clGetDeviceInfo(deviceList[i], CL_DEVICE_VERSION, 256 * sizeof(char), apiVersion,
                                  0);
            if (res != CL_SUCCESS)
                return VKFFT_ERROR_FAILED_TO_ENUMERATE_DEVICES;
...
VkFFTResult performVulkanFFT(VkGPU* vkGPU, VkFFTApplication* app, VkFFTLaunchParams* launchParams,
    ...
    // Record commands num_iter times. Allows to perform multiple convolutions/transforms in one
    // submit.
    ...
    double totTime =
        std::chrono::duration_cast<std::chrono::microseconds>(timeEnd - timeSubmit).count() * 0.001;
...
VkFFTResult performVulkanFFTiFFT(VkGPU* vkGPU, VkFFTApplication* app,
                                 VkFFTLaunchParams* launchParams, uint64_t num_iter,
                                 double* time_result) {
    VkFFTResult resFFT = VKFFT_SUCCESS;
#if (VKFFT_BACKEND == 0)
    VkResult res = VK_SUCCESS;
