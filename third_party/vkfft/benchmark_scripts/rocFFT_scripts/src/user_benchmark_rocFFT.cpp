...
void user_benchmark_rocFFT(bool file_output, FILE* output, rocFFTUserSystemParameters* userParams,
    ...
    if (userParams->R2C) {
        ...
        bufferSize = (uint64_t)storageComplexSize * userParams->X * userParams->Y * userParams->Z *
                     userParams->B;
    }
    ...
    for (int n = 0; n < 2; n++) {
        ...
        for (int r = 0; r < num_runs; r++) {
            ...
            int iembed[2][3];
            int istride[2] = {1, 1};
            int idist[2] = {bufferSize / userParams->B / storageComplexSize,
                            bufferSize / userParams->B / storageComplexSize};
            if (userParams->R2C)
                idist[0] *= 2;
            int oembed[2][3];
            int ostride[2] = {1, 1};
            int odist[2] = {bufferSize / userParams->B / storageComplexSize,
                            bufferSize / userParams->B / storageComplexSize};
            if (userParams->R2C)
                odist[1] *= 2;
            switch (FFTdim) {
            ...
            case 0:
                if (userParams->R2C) {
                    hipfftPlanMany(&plan, FFTdim, dims, iembed[0], istride[0], idist[0], oembed[0],
                                   ostride[0], odist[0], HIPFFT_R2C, userParams->B);
                    hipfftPlanMany(&plan2, FFTdim, dims, iembed[1], istride[1], idist[1], oembed[1],
                                   ostride[1], odist[1], HIPFFT_C2R, userParams->B);
                } else {
                    hipfftPlanMany(&plan, FFTdim, dims, iembed[0], istride[0], idist[0], oembed[0],
                                   ostride[0], odist[0], HIPFFT_C2C, userParams->B);
                }
                break;
            case 1:
                if (userParams->R2C) {
                    hipfftPlanMany(&plan, FFTdim, dims, iembed[0], istride[0], idist[0], oembed[0],
                                   ostride[0], odist[0], HIPFFT_D2Z, userParams->B);
                    hipfftPlanMany(&plan2, FFTdim, dims, iembed[1], istride[1], idist[1], oembed[1],
                                   ostride[1], odist[1], HIPFFT_Z2D, userParams->B);
                } else
                    hipfftPlanMany(&plan, FFTdim, dims, iembed[0], istride[0], idist[0], oembed[0],
                                   ostride[0], odist[0], HIPFFT_Z2Z, userParams->B);
                break;
            }
            ...
            for (int i = 0; i < userParams->N; i++) {
                ...
                case 1:
                    if (userParams->R2C) {
                        ...
                        hipfftExecZ2Z(plan, (hipfftDoubleComplex*)dataC,
                                      (hipfftDoubleComplex*)dataC, -1);
                        hipfftExecZ2Z(plan, (hipfftDoubleComplex*)dataC,
                                      (hipfftDoubleComplex*)dataC, 1);
                    }
                    break;
            ...
            if (n > 0) {
                if (r == num_runs - 1) {
                    ...
                    if (file_output)
                        fprintf(
                            output,
                            "rocFFT System: %" PRIu64 "x%" PRIu64 "x%" PRIu64 " Batch: %" PRIu64
                            " Buffer: %" PRIu64
                            " MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %" PRIu64
                            " benchmark: %" PRIu64 " scaled bandwidth: %0.1f\n",
                            userParams->X, userParams->Y, userParams->Z, userParams->B,
                            bufferSize / 1024 / 1024, avg_time, std_error, userParams->N,
                            (uint64_t)(((double)bufferSize / 1024) / avg_time),
                            ((double)bufferSize / 1024.0 / 1024.0 / 1.024 * 4 * FFTdim / avg_time));

                    printf("rocFFT System: %" PRIu64 "x%" PRIu64 "x%" PRIu64 " Batch: %" PRIu64
                           " Buffer: %" PRIu64
                           " MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %" PRIu64
                           " benchmark: %" PRIu64 " scaled bandwidth: %0.1f\n",
                           userParams->X, userParams->Y, userParams->Z, userParams->B,
