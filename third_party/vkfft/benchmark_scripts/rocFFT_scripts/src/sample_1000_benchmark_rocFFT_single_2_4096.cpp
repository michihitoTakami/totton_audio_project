...
void sample_1000_benchmark_rocFFT_single_2_4096(bool file_output, FILE* output, int device_id) {
    const int num_runs = 5;
    if (file_output)
        fprintf(output,
                "1000 - rocFFT FFT + iFFT C2C benchmark 1D batched in single precision: all "
                "supported systems from 2 to 4096\n");
    printf(
        "1000 - FFT + iFFT C2C benchmark 1D batched in single precision: all supported systems "
        "from 2 to 4096\n");
    ...
    for (int n = 1; n < 4097; n++) {
        ...
        for (int r = 0; r < num_runs; r++) {
            ...
            uint64_t num_iter = ((3 * 4096 * 1024.0 * 1024.0) / rocBufferSize > 1000)
                                    ? 1000
                                    : (3 * 4096 * 1024.0 * 1024.0) / rocBufferSize;
            if (num_iter == 0)
                num_iter = 1;
                ...
                if (r == num_runs - 1) {
                    ...
                    if (file_output)
                        fprintf(
                            ...
                            num_iter, (uint64_t)(((double)rocBufferSize / 1024) / avg_time),
                            rocBufferSize / 1024.0 / 1024.0 / 1.024 * 4 / avg_time);

                    printf("rocFFT System: %" PRIu64 " %" PRIu64 " Buffer: %" PRIu64
                           " MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %" PRIu64
                           " benchmark: %" PRIu64 " bandwidth: %0.1f\n",
                           dims[0], dims[1], rocBufferSize / 1024 / 1024, avg_time, std_error,
                           num_iter, (uint64_t)(((double)rocBufferSize / 1024) / avg_time),
