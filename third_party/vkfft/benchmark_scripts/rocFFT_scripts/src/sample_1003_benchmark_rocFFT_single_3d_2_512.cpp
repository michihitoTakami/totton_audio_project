...
void sample_1003_benchmark_rocFFT_single_3d_2_512(bool file_output, FILE* output, int device_id) {
    if (file_output)
        fprintf(output,
                "1003 - rocFFT FFT + iFFT C2C multidimensional benchmark in single precision: all "
            ...
            if (n > 1) {
                if (r == num_runs - 1) {
                    ...
                    if (file_output)
                        fprintf(
                            output,
                            "rocFFT System: %" PRIu64 " Buffer: %" PRIu64
                            " MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %" PRIu64
                            " benchmark: %" PRIu64 " bandwidth: %0.1f\n",
                            dims[0], rocBufferSize / 1024 / 1024, avg_time, std_error, num_iter,
                            (uint64_t)(((double)rocBufferSize / 1024) / avg_time),
                            3 * rocBufferSize / 1024.0 / 1024.0 / 1.024 * 4 / avg_time);

                    printf("rocFFT System: %" PRIu64 " Buffer: %" PRIu64
                           " MB avg_time_per_step: %0.3f ms std_error: %0.3f num_iter: %" PRIu64
                           " benchmark: %" PRIu64 " bandwidth: %0.1f\n",
                           dims[0], rocBufferSize / 1024 / 1024, avg_time, std_error, num_iter,
                           (uint64_t)(((double)rocBufferSize / 1024) / avg_time),
