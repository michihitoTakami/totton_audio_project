...
void sample_1001_benchmark_rocFFT_double_2_4096(bool file_output, FILE* output, int device_id) {
    ...
    if (file_output)
        fprintf(output,
                "1001 - rocFFT FFT + iFFT C2C benchmark 1D batched in double precision: all "
                "supported systems from 2 to 4096\n");
    printf(
        "1001 - rocFFT FFT + iFFT C2C benchmark 1D batched in double precision: all supported "
