...
class BinaryArchive : public NS::Referencing<BinaryArchive> {
   public:
    ...
    bool addComputePipelineFunctions(const class ComputePipelineDescriptor* descriptor,
                                     NS::Error** error);

    bool addRenderPipelineFunctions(const class RenderPipelineDescriptor* descriptor,
                                    NS::Error** error);

    bool addTileRenderPipelineFunctions(const class TileRenderPipelineDescriptor* descriptor,
                                        NS::Error** error);

    bool serializeToURL(const NS::URL* url, NS::Error** error);

    bool addFunction(const class FunctionDescriptor* descriptor, const class Library* library,
                     NS::Error** error);
};
...
_MTL_INLINE bool MTL::BinaryArchive::addComputePipelineFunctions(
    ...
    return Object::sendMessage<bool>(
        this, _MTL_PRIVATE_SEL(addComputePipelineFunctionsWithDescriptor_error_), descriptor,
        error);
}
